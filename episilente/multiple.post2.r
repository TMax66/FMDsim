multiple.gg.post2=function(x=1,raggio=10,risultati.pre,#casi.scoperti=index.caso.sinto,
...
casi.scoperti=as.character(ll[which(risultati.pre$latenze<sinto)] )
casi.infettanti=as.character(risultati.pre$casi.infettanti[which(risultati.pre$casi.infettanti%nin%casi.scoperti)])
numero.gg=length(risultati.pre$lunghezza)
day=risultati.pre$day
mm=table(tipologia)
if(day.post==1)esclusi=rep(0,dim(dati.fin)[1])else
{
esclusi=risultati.pre$esclusi
casi.scoperti=as.character(unique(c(casi.scoperti,
as.character(casi.infettanti[which(esclusi>0)]))) )
}
...
#n.cod=n.cod[which(n.cod%in%c(1:length(codall)))]
 ...
 for(i in 1:n.infect)
 {
  ...
  pot[n.cod]=NA
  pp1=rowSums(data.frame(pp1,pot),na.rm=T)#pp1=pp1+pot
...
,lunghezza=lunghezza,day=day,esclusi=esclusi,casi.scoperti=as.character(casi.scoperti),
